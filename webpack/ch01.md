...
## 配置映射命令
在package.json中："scripts":{"build":"webpack"}，打包以后执行yarn build即可

（一句话总结src里是前端，外面的全是node）

## 手写配置webpack.config.js（重点）
1.
...
output: { path："" filename："gaiming.js" // 打包输出路径
...
问题引出：我们打包的是js文件，那我们网页的css，还有图片那些怎么办，如何打包呢？！请出loader
...
打包报错！原来webpack默认情况下，只会处理js文件，如果我们希望它可以处理其他类型的文件，则要为其引入loader（图片webpack是支持打包的，我们给他写好规则指好路径如下）
